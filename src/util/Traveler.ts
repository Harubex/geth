...
const debug = new Debug("traveler");
...
export default class Traveler {
     ...
     * @param options
     */
    public static travelTo(creep: Creep, destination: HasPos|RoomPosition, options: TravelToOptions = {}):
        CreepMoveReturnCode | ERR_NO_PATH | ERR_INVALID_ARGS {
        ...
        const travelData = (creep.memory as any)._trav as TravelData;
        ...
        if (!travelData.path) {
            ...
            if (state.cpu > REPORT_CPU_THRESHOLD) {
                ...
                debug.log(`TRAVELER: heavy cpu use: ${creep.name}, cpu: ${state.cpu} origin: ${
            ...
            if (ret.incomplete) {
                ...
                // debug.log(`TRAVELER: incomplete path for ${creep.name}`);
                color = "red";
            }
     ...
     * @param roomName
     */
    public static checkAvoid(roomName: string): RoomMemory | number {
    ...
    public static findTravelPath(origin: RoomPosition|HasPos, destination: RoomPosition|HasPos,
        ...
        if (ret.incomplete && options.ensurePath) {
            ...
            if (options.useFindRoute === undefined) {
                ...
                if (roomDistance <= 2) {
                    debug.log(`TRAVELER: path failed without findroute, trying with options.useFindRoute = true`);
                    debug.log(`from: ${origin}, destination: ${destination}`);
                    options.useFindRoute = true;
                    ret = this.findTravelPath(origin, destination, options);
                    debug.log(`TRAVELER: second attempt was ${ret.incomplete ? "not " : ""}successful`);
                    return ret;
                }
            } else {
    ...
    public static findRoute(origin: string, destination: string,
        ...
        if (!_.isArray(ret)) {
            debug.log(`couldn't findRoute to ${destination}`);
            return;
        }
    ...
    public static patchMemory(cleanup = false) {
        ...
        for (const roomName in Memory.empire.hostileRooms) {
            if (Memory.empire.hostileRooms[roomName]) {
                ...
                (Memory.rooms[roomName] as any).avoid = 1;
                count++;
            }
            if (cleanup) {
        ...
        debug.log(`TRAVELER: room avoidance data patched for ${count} rooms`);
    }
